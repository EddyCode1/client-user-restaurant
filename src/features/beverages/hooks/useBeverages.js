...
/**
 * Hook para obtener y manejar la lista de bebidas (Modo Cliente)
 */
...
/**
 * Hook para obtener una bebida específica por ID
 */
...
export const useSearchBeverages = (restaurantId = null) => {
  ...
  const handleSearch = useCallback(
    async (term) => {
      if (!restaurantId) return [];
      if (!term) return store.beverages;
...
/**
 * Hook para obtener bebidas por tipo (filtradas por disponibilidad)
 */
export const useBeveragesByType = (type) => {
  const { beverages } = useBeverageStore();
  // Se mantiene el filtro de 'available' por ser relevante para el cliente
